...
export const ProblemSubmissionInfo: React.FC<ProblemSubmissionInfoProps> = ({ projectId, submissionUser }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex w-fit items-center gap-3 rounded-md bg-info/10 p-4">
        <UserIcon className="h-6 w-6" />
        <div className="flex flex-col gap-2">
          <span>{submissionUser.username}</span>
          <Badge variant="default" className="flex w-fit items-center gap-1 px-2 py-0.5 text-xs">
            {userRole?.name}
          </Badge>
        </div>
      </div>

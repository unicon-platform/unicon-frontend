...
type SelectedTaskIdContextType = {
  selectedTaskVersionId: number;
  setSelectedTaskVersionId: (id: number) => void;
};

export const SelectedTaskIdContext = createContext<SelectedTaskIdContextType | null>(null);
...
const TaskCard: React.FC<OwnProps> = ({
  ...
  const [selectedTaskVersionId, setSelectedTaskVersionId] = useState(submissionAttempt?.task_id ?? updatedTask.id);
  ...
  return (
    <SelectedTaskIdContext.Provider value={{ selectedTaskVersionId, setSelectedTaskVersionId }}>
            ...
            {!isSelectedVersionUpdated && (
              <Alert variant="destructive" className="mb-2 flex items-center gap-2">
                ...
                <AlertDescription className="flex w-full items-center justify-between">
                  <span>You're viewing an outdated task version.</span>{" "}
                  <Button
                    size="sm"
                    variant={"secondary"}
                    type="button"
